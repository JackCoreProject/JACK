...
public:
    ...
    bool WriteZJACKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZJACKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZJACKSeed_deprecated(uint256& seed);
    bool EraseZJACKSeed();
    bool EraseZJACKSeed_deprecated();

    bool WriteZJACKCount(const uint32_t& nCount);
    bool ReadZJACKCount(uint32_t& nCount);
