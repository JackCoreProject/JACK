...
// The zJACK block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zJACK is a hash of the serial
...
{
    //Create an output returning the zJACK that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZJACKOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zJACK output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zJACK", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZJACKOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zJACK output", __func__);
...
{
    CzJACKTracker* zjackTracker = pwallet->zjackTracker.get();
    CMintMeta meta;
    if (!zjackTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zjackTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a JACK stake is the outpoint
