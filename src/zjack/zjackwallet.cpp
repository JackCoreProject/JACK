...
CzJACKWallet::CzJACKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zjack seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZJACKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZJACKSeed_deprecated()) {
                    LogPrintf("%s: Updated zJACK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zjack seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zjack wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzJACKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZJACKCount(nCountLastUsed);
    else if (!walletdb.ReadZJACKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzJACKWallet::Lock()
{
    seedMaster = 0;
}

void CzJACKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzJACKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZJACK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzJACKWallet::LoadMintPoolFromDB()
{
...
void CzJACKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzJACKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzJACKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zjackTracker->HasPubcoinHash(pMint.first)) {
...
bool CzJACKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZJACK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zjackTracker which also adds to database
    pwalletMain->zjackTracker->Add(dMint, true);
...
void CzJACKWallet::SeedToZJACK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzJACKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzJACKWallet::GenerateDeterministicZJACK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzJACKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzJACKWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzJACKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
