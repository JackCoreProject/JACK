...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZJACKModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zjackTracker->HasMintTx(hash))) {
            //zJACK stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZJACK;
...
bool IsZJACKType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZJACK:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZJACK || type == TransactionRecord::MNReward) {
