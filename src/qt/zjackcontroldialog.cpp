...
#include "ui_zjackcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zjackTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zjackTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zJACK.";
            else if (!mint.isSeedCorrect)
                strReason = "The zJACK seed used to mint this zJACK is not the same as currently hold in the wallet";
            else
