...
#include "zjack/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "JACK cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ujack) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zJACK spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zJACK is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zJACK spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zJACK without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zJACK v1 serial spend not spendable, serial %s, tx %s\n", __func__,
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(v1Serial)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zJACK spend with serial %s from tx %s is not in valid range\n", __func__,
                     spend->getCoinSerialNumber().GetHex(), tx.GetHash().GetHex());
        else
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZJACKModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZJACKModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zJACK spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: AcceptToMemoryPool failed for tx %s, every input must be a zcspend or zcpublicspend", __func__,
                                        tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zjack");
                }
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZJACKModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zJACK mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 0) {
        ...
        nSubsidy = 5.5 * COIN;
    } else if (nHeight <= 64800 && nHeight >= 43201) {
        nSubsidy = 6 * COIN;
    } else if (nHeight <= 86400 && nHeight >= 64801) {
        nSubsidy = 6.5 * COIN;
    } else if (nHeight <= 108000 && nHeight >= 86401) {
        nSubsidy = 7 * COIN;
    } else if (nHeight <= 129600 && nHeight >= 108001) {
        nSubsidy = 7.5 * COIN;
    } else if (nHeight <= 151200 && nHeight >= 129601) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <= 172800 && nHeight >= 151201) {
        nSubsidy = 8.5 * COIN;
    } else if (nHeight <= 194400 && nHeight >= 172801) {
        nSubsidy = 9 * COIN;
    } else if (nHeight <= 216000 && nHeight >= 194401) {
        nSubsidy = 9.5 * COIN;
    } else if (nHeight <= 237600 && nHeight >= 216001) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 259200 && nHeight >= 237601) {
        nSubsidy = 11 * COIN;
    } else if (nHeight <= 280800 && nHeight >= 259201) {
        nSubsidy = 12 * COIN;
    } else if (nHeight <= 302400 && nHeight >= 280801) {
        nSubsidy = 13 * COIN;
    } else if (nHeight <= 324000 && nHeight >= 302401) {
        nSubsidy = 14 * COIN;
    } else if (nHeight <= 345600 && nHeight >= 324001) {
        nSubsidy = 15 * COIN;
    } else if (nHeight <= 367200 && nHeight >= 345601) {
        nSubsidy = 16 * COIN;
    } else if (nHeight <= 388800 && nHeight >= 367201) {
        nSubsidy = 17 * COIN;
    } else if (nHeight <= 410400 && nHeight >= 388801) {
        nSubsidy = 18 * COIN;
    } else if (nHeight <= 432000 && nHeight >= 410401) {
        nSubsidy = 19 * COIN;
    } else if (nHeight <= 453600 && nHeight >= 432001) {
        nSubsidy = 20 * COIN;
    } else if (nHeight <= 475200 && nHeight >= 453601) {
        nSubsidy = 21 * COIN;
    } else if (nHeight <= 496800 && nHeight >= 475201) {
        nSubsidy = 22 * COIN;
    } else if (nHeight <= 518400 && nHeight >= 496801) {
        nSubsidy = 23 * COIN;
    } else if (nHeight <= 540000 && nHeight >= 518401) {
        nSubsidy = 24 * COIN;
    } else if (nHeight <= 561600 && nHeight >= 540001) {
        nSubsidy = 25 * COIN;
    } else if (nHeight <= 583200 && nHeight >= 561601) {
        nSubsidy = 22 * COIN;
    } else if (nHeight <= 604800 && nHeight >= 583201) {
        nSubsidy = 19 * COIN;
    } else if (nHeight <= 626400 && nHeight >= 604801) {
        nSubsidy = 16 * COIN;
    } else if (nHeight <= 648000 && nHeight >= 626401) {
        nSubsidy = 13 * COIN;
    } else if (nHeight <= 669600 && nHeight >= 648001) {
        nSubsidy = 10 * COIN;
    } else if (nHeight <= 691200 && nHeight >= 669601) {
        nSubsidy = 9 * COIN;
    } else if (nHeight <= 712800 && nHeight >= 691201) {
        nSubsidy = 8 * COIN;
    } else if (nHeight <= 734400 && nHeight >= 712801) {
        nSubsidy = 7 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZJACKStake)
{
    int64_t ret = 0;
    ...
    if (nHeight <= 43200) {
        ...
        //When zJACK is staked, masternode only gets 2 JACK
        ret = 3 * COIN;
        if (isZJACKStake)
            ret = 2 * COIN;
    }
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZJACKModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from JACK
...
void RecalculateZJACKMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZJACK..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZJACK..."), percent);
        }
...
void RecalculateZJACKSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZJACK..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZJACK..."), percent);
        }

        //Rewrite zJACK supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zJACK supply
...
bool RecalculateJACKSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating JACK supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating JACK supply..."), percent);
        }
...
{
    // JACK: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZJACKSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zJACK supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zJACK mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZJACKMinted();
        RecalculateZJACKSpent();
        RecalculateJACKSupply(Params().Zerocoin_StartHeight());
    }

    //Track zJACK money supply in the block index
    if (!UpdateZJACKSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zJACK supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert JACK to zJACK
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zJACK spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zJACK serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> jackInputs;
        std::vector<CTxIn> zJACKInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zJACKInputs.push_back(stakeIn);
            }else{
                jackInputs.push_back(stakeIn);
            }
        }
        const bool hasJACKInputs = !jackInputs.empty();
        const bool hasZJACKInputs = !zJACKInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasJACKInputs)
                    ...
                    for (const CTxIn& jackIn : jackInputs){
                        if(jackIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : jackInputs) {
            ...
            // Now that this loop if completed. Check if we have zJACK inputs.
            if(hasZJACKInputs){
                for (const CTxIn& zPivInput : zJACKInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zjack");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zJACK mints and %d zJACK spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // JACK: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zjack", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zjack", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
